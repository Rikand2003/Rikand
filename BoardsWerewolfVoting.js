...
// ==UserScript==
// @name	boards.ie - Mafia Votal Generator
// @namespace	All credit to Makaze - I just edited a couple of lines to try and make it work on boards.ie
// @description	A feature-rich votal generator generator for boards.ie.
// @include	*://www.boards.ie/vbulletin/*
// @grant	none
// @version	1.0.0
// ==/UserScript==
...
function VoteCounter() {
	...
	defaults = {
		...
		'players': {
			'0': { 'aliases': ['No Lynch'], 'id': 0, 'lynch_requirement': 0, 'dead': false, 'postcount': 0 }
		},
		'post': 1,
	...
	vote_check = false,
	end_phase = false,
	thread = location.href.match(/puzzles-other-games\/.*?\-(\d+)(\-post\d+)?(\/|$)/i)[1] || null,
			...
			var switchesChecker = setInterval(function() {
				if (UI_loaded) {
					document.body.appendChild(createElement('script', function(script) {
						...
						script.src = 'https://pastebin.com/raw.php?i=nvhwrEHm';
	...
	function controls(game) {
		...
		postFrame = document.body.appendChild(createElement('iframe', function(frame) {
			frame.src = 'www.boards.ie/vbulletin/showthread.php?threadid=' + game.id;
			frame.id = 'postFrame';
		...
		return createElement('div', function(container) {
			...
			container.appendChild(createElement('div', function(main) {
				...
				main.appendChild(createElement('a', function(timer) {
					...
					timer.title = 'Toggle Automated Posting';
				...
				main.appendChild(createElement('span', function(right) {
					...
					right.appendChild(createElement('a', function(control) {
						...
						control.title = 'Post votals right now';
					...
					right.appendChild(createElement('a', function(control) {
						...
						control.className = 'tabControl';
						control.href = 'javascript:void(0)';
						control.appendChild(document.createTextNode(((game.phase.running) ? 'End' : 'Start') + ' Phase'));
			...
			container.appendChild(createElement('div', function(tabs) {
				...
				tabs.appendChild(createElement('div', function(tab) {
					...
					tab.appendChild(createElement('div', function(setting) {
							...
							submit.onclick = function() {
								...
								if (!game.phase.running) {
									if (!isNaN(endDate)) {
										...
										document.getElementById('PhaseControl').textContent = 'End Phase';
						...
						setting.appendChild(createElement('div', function(helpcont) {
							...
							helpcont.appendChild(createElement('div', function(tip) {
								tip.className = 'tip';
								tip.appendChild(document.createTextNode('Starts or Ends a phase with the current game data.'));
							}));
						}));
					}));
				}));
				...
				tabs.appendChild(createElement('div', function(tab) {
					...
					if (Object.keys(players).length > 1) {
						...
						tab.appendChild(playerField({ 'aliases': [], 'id': '', 'lynch_requirement': 0, 'vote_power': 1, 'show_vote_power': true, 'dead': false, 'postcount': 0 }, i));
					}
					...
					tab.appendChild(createElement('div', function(setting) {
						...
						setting.appendChild(createElement('a', function(add) {
							...
							add.onclick = function() {
								...
								context.insertBefore(playerField({ 'aliases': [], 'id': '', 'lynch_requirement': 0, 'vote_power': 1, 'show_vote_power': true, 'dead': false, 'postcount': 0 }, index), context.getElementsByClassName('playerField')[context.getElementsByClassName('playerField').length -1].nextSibling);
							};
						}));
					}));
					...
					tab.appendChild(createElement('div', function(setting) {
						...
						setting.appendChild(createElement('input', function(submit) {
							...
							submit.onclick = function() {
								...
								playersTemp = { '0': { 'aliases': ['No Lynch'], 'id': 0, 'lynch_requirement': 0, 'dead': false, 'postcount': 0 } },
								aliases,
								player,
								lynchRequirement,
								votePower,
								...
								for (i = 0; i < players.length; i++) {
									...
									playersTemp[player] = { 'aliases': aliases, 'id': player, 'lynch_requirement': lynchRequirement, 'vote_power': votePower, 'show_vote_power': showVotePower, 'dead': deathStatus, 'postcount': 0 };
								}
	...
	function Votal(game, method, endPhase, hammer) {
				...
				if (prop !== '0') {
					if (!game.players[prop].dead) {
						if (!recent.hasOwnProperty(game.players[prop].aliases[0]) || recent[game.players[prop].aliases[0]].vote === 'Unvote') {
							...
							novoters += game.players[prop].aliases[0] + ' (' + game.players[prop].postcount + ')';
							novotes++;
						}
					}
				}
		...
		function voteLinkRecent(j) {
			return createElement('a', function(v) {
				...
				v.href = url;
				v.appendChild(document.createTextNode(voters[j]) + ' (' + game.players[voters[j]].postcount + ')');
			});
		}
		...
		if (jQuery('.cke_contents iframe', postDoc).length) {
			...
			if (cache_field) {
				...
				field.appendChild(createElement('p', function(body) {
					body.appendChild(document.createTextNode('[SPOIL]'));
				}));
		...
		} else if (jQuery('#vB_Editor_QR textarea', postDoc).length) {
			field = jQuery('#vB_Editor_QR textarea', postDoc).get(0);

			field.value = '[U][B]' + game.phase.name + ': Automated Votals from #' + game.last.post + ' to #' + game.post + '[/B][/U]\n\n';
			...
			if (hammer) {
				jQuery.each(votals, function() {
					...
					if (this.trueTally >= hammerThreshold) {
						field.value += '[U][COLOR=#FF0000][B]' + this.user + ' has been Hammered![/B][/COLOR][/U]\n\n';
						return false;
					}
				});
			}
			...
			for (i = 0; i < votals.length; i++) {
				...
				for (j = 0, voters = votals[i].voters; j < voters.length; j++) {
					...
					field.value += '[URL=' + recent[voters[j]].url + ']' + voters[j] + '[/URL] (' + game.players[voters[j]].postcount + ')';
				}
		...
		if (endPhase) {
			toggleAuto('off');
			document.getElementById('PhaseControl').textContent = 'Start Phase';
	...
	function getVotes(loadTo, thread, post, endPhase) {
		...
		jQuery(loadTo).load('www.boards.ie/vbulletin/showthread.php?threadid=' + thread + '&page=' + game.page + ' #posts', function() {
			jQuery(loadTo).find('.smallfont:contains("Quote")').parent().remove();
			jQuery(loadTo).find('font[color="Red"], font[color="#FF0000"]').remove();
			jQuery(loadTo).find('br').replaceWith(document.createTextNode('\n'));

			if (parseInt(jQuery(loadTo).find('a[id*="postcount"]:last').text().trim()) > game.post) {
				jQuery(loadTo).find('table[id*="post"]').each(function() {
					var self = this,
					player = jQuery(self).find('.bigusername').text().trim(),
					playerName,
					postURL = 'www.boards.ie/vbulletin/showthread.php?threadid=' + thread + '&p=' + jQuery(self).find('a[id*="postcount"]').attr('href').match(/p=(\d+)/i)[1],
					postNum = '#' + jQuery(self).find('a[id*="postcount"]').text().trim(),
					postContent = jQuery(self).find('[id*="post_message_"]:first').text(),
					unvote = false,
					vote = false,
					bolds = jQuery(self).find('[id*="post_message_"]:first').find('b'),
					...
					bolds.reverse().each(function() {
						var currAlias = jQuery(this).text().trim();
						
						lynchee = checkAlias(currAlias, players).name;

						if (currAlias == 'Unvote') {
							unvote = true;
							return false;
						} else if (lynchee !== false) {
							vote = lynchee;
							return false;
						}
					});
					...
					if (parseInt(postNum.split('#')[1]) > game.post) {
						...
						if (players[player] && !players[player].dead) {
							...
							game.players[player].postcount++;

							if (vote) {
								if (lynchee !== false && !players[lynchee].dead) {
									if (lynchee !== lastVote) {
									...
									if (votes.history[votes.history.length - 1] != null && postNum > votes.history[votes.history.length - 1].post) {
									...
									if (votes.player_history.hasOwnProperty(playerName)) {
										if (votes.player_history[playerName][votes.player_history[playerName].length - 1] != null && postNum > votes.player_history[playerName][votes.player_history[playerName].length - 1].post) {
			...
			jQuery(loadTo).load('www.boards.ie/vbulletin/showthread.php?threadid=' + thread + '&page=' + game.page + ' .pagenav', function() {
				// do page stuff
				if (jQuery(loadTo).find('a[rel="next"]')[0] != null) {
					game.page++;
					...
					if (post) {
						...
						if (!debug_mode) {
							jQuery('#qr_submit', postFrame.contentWindow.document).click();
						}
						jQuery(notice).fadeOut('fast');
	...
	styleElem.childNodes[0].nodeValue +=
		...
		'.VoteCounterControls .timerSymbol.play {\n' +
			'width: 20px;\n' +
			'height: 20px;\n' +
			'background-image: url(https://i.imgur.com/jf8sKVQ.png);\n' +
		...
		'.VoteCounterControls .timerSymbol.stop {\n' +
			'width: 20px;\n' +
			'height: 20px;\n' +
			'background-image: url(https://i.imgur.com/lrQi4yr.png);\n' +
			'background-position: 50%;\n' +
			'background-repeat: no-repeat;\n' +
			'vertical-align: middle;\n' +
		'}\n\n' +
	...
	jQuery('#poststop:first').after(createElement('li', function(btn) {
		btn.setAttribute('style', "display: inline-block; padding: .5em 1em; background: #eee; margin: 1em 0; border-radius: 3px; border: 1px solid #555; box-shadow: 0px 1px 0px #555;");
		btn.appendChild(createElement('a', function(link) {
			...
			link.onclick = function() {
				if (!jQuery('.VoteCounterControls').length) {
					jQuery('#poststop:first').after(controls(game));
					addSwitches();
					addTips();
					jQuery('html, body').animate({
						scrollTop: jQuery('.VoteCounterControls:first').offset().top
